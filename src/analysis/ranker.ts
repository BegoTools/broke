...
const CONF_RANK: Record<MediaCandidate["confidence"], number> = {
  high: 3,
  medium: 2,
  low: 1,
  unknown: 0,
...
const STATUS_RANK: Record<MediaCandidate["status"], number> = {
  active: 5,
  pending: 3,
  "auth-required": 2,
  inactive: 1,
...
export function rankCandidates(candidates: MediaCandidate[]): MediaCandidate[] {
...
function score(c: MediaCandidate): number {
  ...
  if (c.container !== "unknown") s += 3;
  else s += 1;
  // Is video content
  if (c.kind === "video") s += 2;
  else if (c.kind === "audio") s += 1;
  // Resolution / quality (highest height wins)
  const maxH = Math.max(0, ...c.qualities.map((q) => q.height ?? 0));
  if (maxH >= 1080) s += 4;
  else if (maxH >= 720) s += 3;
  else if (maxH >= 480) s += 2;
  else if (c.qualities.length > 0) s += 1;
  // Playback compatibility: manifest types & browser-native containers rank high
  if (c.manifestType === "hls" || c.manifestType === "dash") s += 2; // standard, supports qualities
  if (c.container === "mp4" || c.container === "webm") s += 2;
  // Directness: media files more direct than manifests
  if (c.manifestType === "none" && c.container !== "unknown") s += 1;
  // Active state
  s += STATUS_RANK[c.status] ?? 0;
  // Auth dependency is a slight negative (session-bound)
  if (c.authRequired) s -= 1;
...
export function bestPlaybackUrl(c: MediaCandidate): string {
