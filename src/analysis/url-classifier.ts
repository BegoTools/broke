import type { ContainerType, ManifestType, MediaKind } from "../shared/types.js";
...
export function classifyManifestType(container: ContainerType): ManifestType {
  switch (container) {
    case "hls":
      return "hls";
    case "dash":
...
export function classifyKind(mime?: string, container?: ContainerType): MediaKind {
  if (mime) {
    const m = mime.toLowerCase();
    if (m.startsWith("audio/")) return "audio";
    if (m.startsWith("video/")) return "video";
  }
  if (container && (container === "mp3" as ContainerType || container === "ogg")) {
