...
const SECRET_KEY_RE =
  /(authorization|auth[-_]?token|access[-_]?token|refresh[-_]?token|bearer|session[-_]?id|sessionid|cookie|set[-_]?cookie|csrf|xsrf|password|passwd|secret|api[-_]?key|apikey|private[-_]?key|x-amz-security-token|id[-_]?token|client[-_]?secret)/i;
