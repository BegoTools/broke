import type { MediaCandidate, MonitorState, DebugLogEntry } from "../shared/types.js";
import type {
  FromPopup,
  FromContent,
...
import { chatRoute, type ToolName } from "../ai/chat.js";

// Background orchestrator: receives raw candidates from content scripts,
// performs safe manifest analysis, normalizes, deduplicates, ranks, and
// streams results to the popup. State is per-tab in memory + persisted logs.
...
async function debugLog(tabId: number, msg: string, level: DebugLogEntry["level"] = "info") {
  const s = getSession(tabId);
  const entry: DebugLogEntry = { t: Date.now(), level, msg: redactLog(msg) };
  s.debug.push(entry);
...
async function sendState(tabId: number) {
  const s = getSession(tabId);
  broadcast({
    type: "STATE",
    state: s.state,
...
async function sendCandidates(tabId: number) {
  const s = getSession(tabId);
...
async function handleCandidateBatch(tabId: number, candidates: MediaCandidate[]) {
  const s = getSession(tabId);
...
async function startDetection(tabId: number) {
  const s = getSession(tabId);
  try {
...
async function stopDetection(tabId: number) {
  const s = getSession(tabId);
  try {
...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name === "vsd-popup") {
    popupPorts.add(port);
    port.onMessage.addListener(async (msg: FromPopup) => {
    ...
    port.onMessage.addListener(async (msg: FromContent) => {
      if (msg.type === "CANDIDATE_BATCH") {
        ...
        const tabId = port.sender?.tab?.id;
        if (typeof tabId === "number") await debugLog(tabId, msg.entry.msg, msg.entry.level);
...
async function routePopup(port: chrome.runtime.Port, msg: FromPopup) {
  switch (msg.type) {
    ...
    case "STOP_DETECTION":
      ...
      for (const id of sessions.keys()) await stopDetection(id);
      break;
    case "GET_STATE": {
    ...
    case "CLEAR_DEBUG_LOGS":
      ...
      for (const s of sessions.values()) s.debug = [];
      port.postMessage({ type: "DEBUG_LOGS", logs: [] });
      break;
    case "GET_SETTINGS": {
      const settings = await loadSettings();
...
async function lastActiveTab(): Promise<number | null> {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  return tab && typeof tab.id === "number" ? tab.id : null;
}

async function handleDownload(port: chrome.runtime.Port, url: string) {
  if (!isSafeUrl(url)) {
    port.postMessage({ type: "ERROR", message: "Refusing to download unsafe URL." });
    return;
  }
  try {
    const opts: chrome.downloads.DownloadOptions = { url, saveAs: true };
...
async function handleOpen(port: chrome.runtime.Port, url: string) {
  if (!isSafeUrl(url)) {
...
async function maybeAutoAnnotate(tabId: number) {
  const settings = await loadSettings();
  if (!settings.aiEnabled || !settings.aiAutoAnnotate || !settings.geminiApiKey) return;
  try {
    const s = getSession(tabId);
    ...
    for (const e of enriched) s.dedup.add(e);
    await sendCandidates(tabId);
...
async function runAnnotation(port: chrome.runtime.Port) {
  const settings = await loadSettings();
...
async function runChat(port: chrome.runtime.Port, prompt: string) {
  const settings = await loadSettings();
  if (!settings.aiEnabled || !settings.geminiApiKey) {
...
async function executeTool(port: chrome.runtime.Port, intent: { tool: ToolName; args: Record<string, unknown> }) {
  switch (intent.tool) {
    case "start_detection": {
    ...
    case "stop_detection":
      ...
      break;
    case "open_url":
      if (intent.args.url) await handleOpen(port, String(intent.args.url));
      break;
    case "download_url":
      if (intent.args.url) await handleDownload(port, String(intent.args.url));
      break;
    case "navigate": {
      const id = (await lastActiveTab()) ?? -1;
      if (id >= 0 && intent.args.url) await chrome.tabs.update(id, { url: String(intent.args.url) });
      break;
    }
    case "select_quality": {
      ...
      if (top) {
        ...
        if (q) ss.selectedQuality = top.id + "|" + q.label;
      }
      break;
    }
  }
...
async function findTabByQuery(query: string): Promise<number | null> {
...
chrome.commands.onCommand.addListener(async (command) => {
  if (command === "start_detection") {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab && typeof tab.id === "number") await startDetection(tab.id);
  }
});
