...
// Natural-language router for the in-extension chat.
// - Command-like prompts (e.g. Arabic "خش افحص على 123") resolve to a tool intent.
// - Free-form questions are answered by Gemini with allowlisted function-calling.
...
export function localRoute(prompt: string): ChatIntent | null {
  const p = prompt.trim();
...
export async function chatRoute(
  ...
  if (local) return { intent: local, reply: "" };
  ...
  if (tool) return { intent: tool, reply: stripToolJson(res.text) };
  return { intent: { kind: "chat", prompt }, reply: res.text };
}

function extractToolCall(text: string): ChatIntent | null {
  ...
  if (m && TOOLS.some((t) => t.name === m[1])) {
    try {
      ...
      return { kind: "tool", tool: m[1] as ToolName, args: {} };
    }
  }
  return null;
}
