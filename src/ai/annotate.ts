...
// Builds a compact, privacy-safe prompt and asks Gemini to label each link.
// Secret param values are masked before leaving the extension.

export interface AiAnnotation {
  id: string;
  label: string; // e.g. "player", "manifest", "download", "thumbnail", "analytics"
  summary: string;
}
...
export function buildAnnotationPrompt(candidates: MediaCandidate[]): string {
  ...
  return (
    "Classify these detected links:\n" + JSON.stringify(items, null, 2)
  );
}

// Parse Gemini's JSON array response. Tolerant of surrounding prose.
export function parseAnnotations(text: string): AiAnnotation[] {
  let json = text.trim();
